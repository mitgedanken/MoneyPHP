...
/**
 * @deprecated since version 0.0.1-alpha+2714m0
 * A set of algorithms.
 *
...
class Algorithms {
     ...
     *
     * @var integer [default: Interfaces\Money::HALF_EVEN]
     */
    ...
    public static function divide($dividend, $divisor, $scale)
    {
        if (!\is_numeric($dividend)):
            $message = 'Argument 1 is not numeric' . InvalidArgument::was($dividend);
            throw new Exceptions\InvalidArgument($message);
        endif;
        if (!\is_numeric($divisor)):
            $message = 'Argument 2 is not numeric' . InvalidArgument::was($divisor);
            throw new Exceptions\InvalidArgument($message);
        endif;
        if (!\is_integer($scale)):
            $message = 'Argument 3 is not an integer' . InvalidArgument::was($scale);
            throw new Exceptions\InvalidArgument($message);
        endif;
        if (isset(static::$divide_algo)):
            ...
            if ($result === NULL):
                throw new Exeptions\DivisionByZero();
            endif;
        endif;
        return $result;
    }
     ...
     * @param numeric_string $number
     * @param boolean $rounding [default: FALSE] rounds the result if </i>TRUE</i>
     * @return Interfaces\Money[] the allocated monies.
     */
    public static function allocate($ratios, $number, $rounding = FALSE)
    {
        if (isset(static::$allocate_algo)):
            ...
            \settype($number, ContainerInterface::DEFAULT_SETTYPE_TYPE);
